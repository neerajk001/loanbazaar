...
import { X, User, Phone, IndianRupee, Shield, CheckCircle } from 'lucide-react';
...
const InsuranceApplicationForm = ({ insuranceType = 'health' }: InsuranceApplicationFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Insurance form submit error:', error);
  ...
  return (
    <div className="bg-white w-full rounded-t-[2rem] md:rounded-3xl shadow-2xl overflow-hidden h-[90vh] md:h-auto md:max-h-[90vh] flex flex-col">
      ...
      <div className="bg-blue-50 border-b border-gray-200 px-4 py-4 md:px-8">
        <div className="max-w-2xl mx-auto text-center">
          <Shield className="w-8 h-8 text-blue-600 mx-auto mb-2" />
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 capitalize">{insuranceType.replace('-', ' ')} Insurance</h1>
        ...
        <div className="max-w-2xl mx-auto">
          {showSuccess ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-800 font-semibold">Quote request submitted successfully.</p>
            </div>
          ) : (
