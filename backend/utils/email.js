...
function createLoanApplicationConfirmationEmail(name, applicationId, email, loanType, loanAmount) {
...
function createInsuranceApplicationConfirmationEmail(name, applicationId, email, insuranceType) {
...
function createStatusUpdateEmail(name, applicationId, email, oldStatus, newStatus, notes) {
  const statusColors = {
    pending: '#f59e0b',
    reviewing: '#3b82f6',
    verified: '#8b5cf6',
...
function createAdminNotificationEmail(adminEmail, applicationId, applicantName, type) {
  ...
  const html = `
    ...
    <body style="font-family: Arial, sans-serif;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <a href="${process.env.FRONTEND_URL || 'http://localhost:3000'}/admin/applications" style="display: inline-block; padding: 12px 30px; background: #667eea; color: white; text-decoration: none; border-radius: 5px; margin-top: 20px;">View in Admin Panel</a>
      </div>
    </body>
    </html>
  `;
...
View in Admin Panel: ${process.env.FRONTEND_URL || 'http://localhost:3000'}/admin/applications
  `;
